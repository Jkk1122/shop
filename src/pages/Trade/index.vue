<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <!-- 收件人信息 -->
      <div class="address clearFix" v-for="(user) in userAddressList" :key="user.id">
        <span class="username" :class="{selected:user.isDefault == 1}">{{user.consignee}}</span>
        <p @click="ChooseAddress(user,userAddressList)">
...
import { mapGetters,mapState } from 'vuex';
  export default {
    ...
    // 计算属性
    computed:{
      ...mapGetters(['userAddressList','detailArrayList','totalAmount','totalNum','tradeNo']),
      //捞取选中的地址人员信息
      userDefaultAddress(){
    ...
    methods:{
      //选择地址
      ChooseAddress(user,userAddressList){
        // alert(index)
        if(userAddressList){
      ...
      async SubmitTrade(){
       ...
       let result =  await this.$API.reqSubmitTrade(this.tradeNo,data)
        if(result.code == 200){
